...
export default function HeaderDesktop({
  ...
  return (
    <div
      ...
      className="data-[nav-open=true]:fixed data-[nav-open=true]:h-screen
      data-[nav-open=true]:w-[100%] data-[nav-open=true]:max-w-[100%]
      data-[nav-open=true]:!bg-porcelain">
      <BoxedContainer>
        <div
          data-at-top={atTop}
          data-dark-nav={isDarkNav}
          className="hidden flex-row justify-between gap-x-[20px] pb-7.5
          pt-12 transition-all duration-300 lg:flex lg:gap-x-[50px] 5xl:pt-18.5">
          <a href={`/${lang}` || '#'} className="header-logo">
            {visibleLogo && (
              <div
                className={`transition-opacity duration-300 ease-in-out 
        ${logoOpacity === 1 ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="flex h-fit flex-row gap-x-4 pt-1.5 lg:gap-x-6.5 xl:gap-x-8">
            {_.isEmpty(productsNavigation) || (
              <button
                ...
                data-dark-nav={isDarkNav}
                className={`header-product-navigation-button h-fit font-noto  text-[18px]
                font-medium capitalize text-primary transition-[color,opacity] duration-300 
                ease-in-out data-[selected=true]:!text-cherry
                ${homePage ? 'data-[home-top=true]:text-white' : ''}
                data-[dark-nav=true]:!text-primary
              ...
              if (hasInnerLinks) {
                return (
                  <button
                    ...
                    data-dark-nav={isDarkNav}
                    className="h-fit font-noto text-[18px] font-medium text-primary
                    transition-[color,opacity] duration-300 ease-in-out 
                    data-[dark-nav=true]:!text-primary
                    data-[home-top=true]:text-white data-[open=true]:!text-primary
            ...
            {_.isEmpty(applicationsNavigation) || (
              <button
                ...
                data-home-top={atTop}
                data-dark-nav={isDarkNav}
                className={`header-application-navigation-button h-fit font-noto text-[18px] 
                  font-medium text-primary transition-all duration-300 
                  data-[selected=true]:!text-cherry
                ${homePage ? 'data-[home-top=true]:text-white' : ''} 
                data-[dark-nav=true]:!text-primary
                data-[open=true]:!text-primary`}>
                ...
                if (hasInnerLinks) {
                  return (
                    <button
                      ...
                      data-home-top={atTop && homePage}
                      data-dark-nav={isDarkNav}
                      className="header-innerlinks-navigation-button h-fit font-noto 
                      text-[18px] font-medium
                      text-primary transition-all duration-300 data-[dark-nav=true]:!text-primary
                      data-[home-top=true]:text-white data-[open=true]:!text-primary
                      data-[selected=true]:!text-cherry">
                      {groupName}
                    </button>
            ...
            {_.isEmpty(search) || (
              <>
                ...
                <Link
                  ...
                  data-open={navOpen}
                  data-home-top={atTop}
                  className={`search-button h-fit 
                    svg-child-circle:stroke-primary svg-child-path:stroke-primary 
                    [&>svg>circle]:transition-all [&>svg>circle]:duration-300
                    [&>svg>path]:transition-all [&>svg>path]:duration-300
                ${navOpen ? 'svg-child-circle:!stroke-primary svg-child-path:!stroke-primary' : ''}
                ${atTop && homePage && !isDarkNav ? '[&>svg>circle]:stroke-white [&>svg>path]:stroke-white' : ''}
                ${isDarkNav && homePage ? '[&>svg>circle]:stroke-black [&>svg>path]:stroke-black' : ''}
                `}>
                  <Icons name="Search" />
                </Link>
              </>
