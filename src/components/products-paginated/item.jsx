...
function Item({ item, currProduct, index, handleClick, width, category }) {
  ...
  const isHarvester = selectedCategory === 'Harvesters';
  ...
  const getPowerValue = (_item) => {
    ...
    const enginePowerSpec = _.find(specs, (val) =>
      _.isEqual(_.get(val, 'content.properties.title'), 'Engine power'),
    );

    const langMapping = {
      en: { value: 'valueUS', unit: 'unitUS' },
    ...
    const resultUnit = _.get(enginePowerSpec, `content.properties.${unit}`, '');
    const resultValue = _.get(enginePowerSpec, `content.properties.${value}`, '');
    // console.log(enginePowerSpec);

    return _.trim(`${resultValue} ${resultUnit}`);
  };

  const calculateOffsetLeft = (el) => {
    const currentElement = el;
    const parentElement = _.get(currentElement, 'parentElement');
        ...
        <button
          ...
          <div className=" relative flex flex-col justify-end ">
            {_.isEmpty(item.properties.productTag) || (
              <div className="absolute top-0 flex flex-wrap gap-1.5 md:flex-wrap">
                {_.map(item.properties.productTag, (productTag, productTagIndex) => (
                  <div
                    key={productTagIndex}
                    className={`${productTag === 'New' ? 'bg-[#CC9E00] uppercase' : 'bg-[#71A330]'} flex  w-fit items-center justify-center rounded-[4px] px-3 py-1.5 font-noto text-[13px] font-medium text-white`}>
          ...
          <div className="flex flex-col items-start gap-y-2.5">
            <h4 className="font-noto text-clamp18to21 font-bold leading-1.125 text-primary">
              {`${item.properties.title} ${getProductPower(item, lang, category, isKO)}`}
            </h4>
            ...
            item.properties.category !== 'Rice Transplanters' ? (
              <p className="font-noto text-clamp14to15 text-primary">
                {`${!isEmpty(item.properties.series) ? getTranslationByKey(item.properties.series, translations, lang) : ''}`}
                {`${!isEmpty(item.properties.subCategory) ? `, ${getTranslationByKey(item.properties.subCategory, translations, lang)}` : ''}`}
                {`${!isEmpty(item.properties.category) ? ` ${getTranslationByKey(item.properties.category, translations, lang)}` : ''}`}
              </p>
            ) : (
              <p className="font-noto text-clamp14to15">
                {!isEmpty(item.properties.productCategory)
                  ? `${getTranslationByKey(item.properties.productCategory, translations, lang)}`
                  : ''}
              </p>
            )}
          </div>
          ...
          <div className="relative flex w-full flex-col gap-y-3 bg-white pt-12">
            ...
            {currProduct && currProduct.properties && (
              <div
                ...
                before:w-full before:-translate-x-1/2 before:-translate-y-full before:bg-porcelain before:py-5
                before:content-[''] md:gap-y-6 md:px-10 md:pb-10 before:md:py-9">
                <div className="flex flex-col gap-y-2">
                  <div className="flex flex-row items-center gap-2">
                    <h4 className="z-10 font-noto text-clamp18to21 font-bold leading-1.125 text-primary">
                      ...
                      {` ${getProductPower(item, lang, category, isKO)}`}
                    </h4>
                    {_.isEmpty(item.properties.productTag) || (
                      <div className="flex flex-wrap gap-1.5">
                        {_.map(item.properties.productTag, (productTag, productTagIndex) => (
                          <div
                            key={productTagIndex}
                            className={`${productTag === 'New' ? 'bg-[#CC9E00] uppercase' : 'bg-[#71A330]'} flex items-center justify-center rounded-[4px] px-3 py-1.5 font-noto text-[13px] font-medium text-white`}>
                            {getTranslationByKey(productTag, translations, lang)}
                          </div>
                        ))}
                      </div>
                ...
                <div className="flex flex-col gap-x-10 gap-y-2 md:flex-row md:gap-y-4">
                  ...
                  />
                  {configureParentProducts === null &&
                    (isTractor || (isHarvester && !_.isEmpty(modelId))) &&
                    !shouldExcludeBtn &&
                    currProduct.properties.product3DId !== null && (
                      <Button
                        variant="primaryWhite"
                        target="_blank"
                        label={getTranslationByKey(
                          isTractor ? 'CONFIGURE YOUR TRACTOR' : 'EXPLORE 3D MODEL',
                          translations,
                          lang,
                        )}
                        url={getProductBuildYourOwnLink(lang, modelName, modelId, isTractor)}
                      />
                    )}
                  {configureParentProducts !== null &&
                    (isTractor || (isHarvester && !_.isEmpty(modelId))) &&
                    !shouldExcludeBtn && (
                      <button
                        ...
                        className="inline-flex min-h-[45px] cursor-pointer items-center justify-center whitespace-pre border border-none bg-white px-[32px] py-[12px] text-clamp14to15 font-bold uppercase not-italic leading-1.625 text-primary transition-all duration-300 ease-in-out hover:!border hover:!border-b-white hover:!bg-mercury hover:text-primary disabled:pointer-events-none disabled:opacity-10 md:px-[60px] md:py-[20px] lg:min-h-[64px] lg:px-[85px]">
                        {getTranslationByKey(
                          isTractor ? 'CONFIGURE YOUR TRACTOR' : 'EXPLORE 3D MODEL',
                          translations,
                          lang,
