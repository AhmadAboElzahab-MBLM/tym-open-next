...
import { map, isEmpty, get, isEqual, find } from 'lodash';
...
function Item({ item, currProduct, index, handleClick, width }) {
  ...
  const isOpen = isEqual(item, currProduct);
  const { toggleCursor } = useHoverCursor();
  const selectedCategory = get(currProduct, 'properties.category', '');
  const isTractor = selectedCategory === 'Tractors';
  const model = get(currProduct, 'properties.title', '');
  const modelName = get(currProduct, 'name', '');
  const modelId = get(currProduct, 'properties.product3DId', '');
  const compatibleProduct = get(item, 'properties.compatibleProduct', null);
  const [offsetToLeft, setOffsetToLeft] = useState(0);

  const getPowerValue = (_item) => {
    const specs = get(_item, 'properties.specifications.items', []);
    const product = find(specs, (val) => isEqual(val.content.properties.title, 'Engine power'));
  ...
  return (
    <motion.div
      ...
      <button
        type="button"
        disabled={isEqual(item, currProduct)}
        ...
        <div className="flex flex-col items-start gap-y-2.5">
          ...
          <div className="inline text-start font-noto text-clamp12to15 text-primary">
            {!isEmpty(compatibleProduct) && (
              <>
                {getTranslationByKey('Compatible', translations, lang)}:{' '}
                {map(
                  compatibleProduct
                    .filter((compatibleProductItem) => {
                      if (lang === 'en') {
                        return compatibleProductItem.startsWith('INT:');
                      } if (lang === 'en-us') {
                  ...
                  (filteredItem, index1, array) => (
                    <span key={index1}>
                      {filteredItem.split(':')[1] || filteredItem}
                    ...
                    isEmpty(selectedThumbnail) || (
                      <Image
                        ...
                        className={`!relative !z-10 !object-contain ${
                          selectedIndex === 1 ? 'max-w-[50%]' : 'max-w-[40%]'
                        }`}
                      />
                    ),
                )}
          ...
          {currProduct && currProduct.properties && (
            <div
              ...
              <div className="flex flex-col gap-y-2">
                ...
                text-primary">
                  {`${currProduct.properties.title}`}
                </h4>
                <div className="inline text-start font-noto text-clamp12to15 text-primary">
                  {!isEmpty(currProduct.properties.compatibleProduct) && (
                    <>
                      {getTranslationByKey('Compatible', translations, lang)}:{' '}
                      {map(
                        currProduct.properties.compatibleProduct
                          .filter((compatibleProductItem) => {
                            if (lang === 'en') {
                              return compatibleProductItem.startsWith('INT:');
                            } if (lang === 'en-us') {
              ...
              <div className="flex flex-col gap-x-10 gap-y-2 md:gap-y-4 md:flex-row">
                <Button
                  ...
                  text="attachment-show-more-button"
                ...
                {isTractor && (
                  <Button
                    ...
                    text="attachment-configure-your-tractor-button"
