...
export default function HomeTractorSliderList({ data, text, className }) {
  ...
  return (
    <Link
      href={path || '#'}
      className={`home-product-slider-list-link flex w-full flex-col
      md:gap-y-5
      ${className}
      `}>
      ...
      <div className="flex flex-col gap-y-[10px]">
        <h3 className="font-noto text-[18px] md:text-[21px] font-bold leading-[24px] text-primary">
          {title} {getProductPower(data, lang, category, isKO)}
        </h3>
        <p className="font-noto text-[13px] md:text-[15px] font-normal leading-1 text-primary">{text}</p>
      </div>
    </Link>
  );
}
